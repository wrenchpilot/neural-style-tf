...
if [ "$#" -le 1 ]; then
   echo "Usage: bash stylize_video.sh <path_to_video> <path_to_style_image> <image_format (ppm,jpg,png)> <optimizer (lbfgs,adam)"
   exit 1
fi
...
$FFMPEG -v quiet -i "$1" "${temp_dir}/frame_%04d.${image_format}"
...
num_frames=$(find "$temp_dir" -iname "*.${image_format}" | wc -l)

echo "Computing optical flow [CPU]. This will take a while..."
cd ./video_input
bash make-opt-flow.sh ${content_filename}/frame_%04d.${image_format} ${content_filename}
cd ..
...
$FFMPEG -v quiet -i ./video_output/frame_%04d.${image_format} ./video_output/${content_filename}-stylized.$extension
